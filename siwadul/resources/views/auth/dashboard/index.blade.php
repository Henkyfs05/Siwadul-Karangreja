...
<div class="main-content">
  <section class="section">
    ...
    <h1>Statistik Pengaduan Masyarakat</h1>
                ...
                <tbody>
                  <tr>
                    ...
                    <td class="text-center">
                      ...
                      @if ((auth()->user()->level == 'user' && $p->user_id == auth()->user()->id) || auth()->user()->level == 'admin')
...
document.addEventListener('DOMContentLoaded', () => { 
  const pengaduanByMonth = {!! json_encode($pengaduanByMonth) !!};
  const pengaduanApprovedByMonth = {!! json_encode($pengaduanApprovedByMonth) !!};
  const pengaduanRejectedByMonth = {!! json_encode($pengaduanRejectedByMonth) !!};
  
  const month = Array(12).fill(0).map((_,i) => pengaduanByMonth[i+1] || 0)
  const approved = Array(12).fill(0).map((_,i) => pengaduanApprovedByMonth[i+1] || 0)
  const rejected = Array(12).fill(0).map((_,i) => pengaduanRejectedByMonth[i+1] || 0)
